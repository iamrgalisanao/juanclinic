...
trait AuditLogTrait
{
    ...
    protected static function logEvent($event, $model)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::error("Audit Log Event Failure: " . $e->getMessage(), [
                'event' => $event,
    ...
    public function logAudit($event, $payload = null)
    {
        try {
            ...
            \App\Models\AuditLog::create([
                'tenant_id' => $tenantId,
                'user_id' => \Illuminate\Support\Facades\Auth::id() ?? ($this instanceof \App\Models\User ? $this->id : null),
                'event' => $event,
            ...
            \Illuminate\Support\Facades\Log::error("Manual Audit Log Failure: " . $e->getMessage(), [
                'event' => $event,
                'payload' => $payload
            ]);
        }
