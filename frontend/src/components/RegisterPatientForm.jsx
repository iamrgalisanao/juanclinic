...
const RegisterPatientForm = ({ onPatientAdded, onClose, activeTenant }) => {
    ...
    return (
        <div className="space-y-6 sm:space-y-10">
            {/* Header */}
            <div>
                <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight pr-12 sm:pr-0">New Patient Registry</h2>
                <p className="text-[10px] sm:text-xs font-bold text-slate-400 mt-2 uppercase tracking-widest leading-relaxed">Please fulfill all clinical requirements</p>
            </div>

            {/* Stepper */}
            <div className="relative flex justify-between items-center px-1 sm:px-2">
                <div className="absolute left-0 right-0 h-0.5 bg-his-slate-50 top-[24px] z-0" />
                {steps.map((s) => (
                    <div key={s.id} className="relative z-10 flex flex-col items-center gap-2 group translate-z-0 min-w-0 flex-1">
                        <div
                            className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-500 border-[3px] sm:border-4 ${step >= s.id
                        ...
                        <span className={`text-[8px] sm:text-[10px] font-black uppercase tracking-tight sm:tracking-widest text-center leading-tight max-w-[60px] sm:max-w-none ${step >= s.id ? 'text-his-green-500' : 'text-slate-300'}`}>
                            {s.name}
                        </span>
                    </div>
                ))}
